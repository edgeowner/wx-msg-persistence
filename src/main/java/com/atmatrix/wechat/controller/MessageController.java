...
public class MessageController {
    ...
    @Autowired
    private WechatCommonApplication wechatCommonApplication;
    ...
    @CustomerLog(value = "群信息消息接口")
    @ApiOperation(value = "群信息消息接口", notes = "群信息接收消息")
    @RequestMapping(value = "/send/group", method = RequestMethod.POST)
    @ResponseBody
    public ResponseEntity<Map<String, Object>> groupMemberReceive(@RequestBody WechatReqDataDto wechatReqDataDto) {
        try {
            wechatCommonApplication.batchSaveGroupMemberData(wechatReqDataDto);
            return WebUtil.success("接收群信息成功");
        } catch (Exception e) {
            e.printStackTrace();
            log.error(MessageFormat.format("MQ Send Failed :{0}", e));
            return WebUtil.error("消息接收失败");
